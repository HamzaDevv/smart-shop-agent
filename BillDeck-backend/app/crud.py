...
def get_customer_by_id(db: Session, customer_id: int):
    return db.query(models.Customer).filter(models.Customer.cust_id == customer_id).first()

def get_customer_by_phone(db: Session, phone: str):
    return db.query(models.Customer).filter(models.Customer.phone_no == phone).first()

def get_all_customers(db: Session):
    return db.query(models.Customer).all()

def get_profit_loss_by_date(db: Session, target_date: date):
    """Get all profit/loss entries for sales on a specific date."""
    return (
        db.query(models.ProfitLoss)
        .join(models.SalesData, models.ProfitLoss.sales_id == models.SalesData.sales_id)
        .filter(models.SalesData.transaction_date == target_date)
        .all()
    )
